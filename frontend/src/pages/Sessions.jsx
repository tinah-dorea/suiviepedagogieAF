  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const updatedFormData = {
        nom_session: formData.nom_session?.trim() || null,
        mois: formData.mois?.trim().toLowerCase() || '',
        ...
        date_fin_inscription: formData.date_fin_inscription || null,
        date_debut: formData.date_debut,
        date_fin: formData.date_fin,
        date_exam: formData.date_exam || null,
        duree_cours: formData.duree_cours ? parseInt(formData.duree_cours, 10) : 0
      };

      console.log('Submitting Session data:', updatedFormData);

      if (currentSession) {
        console.log('Updating session:', currentSession.id);
        ...
        console.log('Create response:', newSession);
    ...
    return (
      <div className="p-4 rounded-2xl flex items-start gap-3" style={{ backgroundColor: '#FEF2F2', color: '#DC2626' }}>
        ...
        <div>
          ...
          <span>Impossible de charger les sessions: {error}</span>
        </div>
      </div>
    );
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex gap-2">
            <select
              className="block px-4 py-3 rounded-xl border-2 focus:outline-none focus:ring-2 focus:ring-opacity-50 transition-all"
              style={{ borderColor: COLORS.border, backgroundColor: COLORS.card, color: COLORS.text }}
              value={filterStatus}
        ...
        {filteredSessions.length === 0 && (
          <div className="text-center py-16">
            <CalendarIcon className="mx-auto h-16 w-16 opacity-30 mb-4" style={{ color: COLORS.primary }} />
            <p className="text-lg font-medium" style={{ color: COLORS.text }}>Aucune session trouvée</p>
            <p className="text-sm mt-1" style={{ color: COLORS.textLight }}>Essayez de modifier vos critères de recherche</p>
          </div>
        )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="md:col-span-2">
                    <h4 className="text-sm font-semibold mb-4 pb-2 border-b flex items-center" style={{ borderColor: COLORS.border, color: COLORS.text }}>
                      <span className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold mr-3" style={{ backgroundColor: COLORS.statBlue, color: COLORS.statBlueText }}>1</span>
        ...
        {showEditForm && (
          <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 overflow-y-auto p-4">
            <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full mx-auto my-8 overflow-hidden">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleFormSubmit(e); }} className="p-6 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {/* Section 1 */}
                  <div className="md:col-span-2">
                    <h4 className="text-sm font-semibold mb-4 pb-2 border-b flex items-center" style={{ borderColor: COLORS.border, color: COLORS.text }}>
                      <span className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold mr-3" style={{ backgroundColor: COLORS.statOrange, color: COLORS.statOrangeText }}>1</span>
                      Informations générales
                    </h4>
                  </div>
