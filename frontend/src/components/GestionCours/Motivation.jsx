...
import { EllipsisHorizontalIcon, PencilIcon, TrashIcon, HeartIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const Motivation = () => {
  ...
  const [formData, setFormData] = useState({ nom_motivation: '' });
  ...
  const loadMotivations = async () => {
    try {
      const data = await motivationService.getAll();
      setMotivations(data);
      setLoading(false);
    } catch (error) {
      console.error('Erreur lors du chargement des motivations:', error);
      toast.error('Erreur lors du chargement des motivations');
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (!formData.nom_motivation.trim()) {
        toast.error('Le nom de la motivation est requis');
        return;
      }
      if (currentMotivation) {
        await motivationService.update(currentMotivation.id, formData);
        toast.success('Motivation modifiée avec succès');
      } else {
        await motivationService.create(formData);
        toast.success('Motivation créée avec succès');
      }
  ...
  const handleEdit = (motivation) => {
    ...
    setFormData({ nom_motivation: motivation.nom_motivation });
    setIsModalOpen(true);
  };

  const handleAdd = () => {
    setCurrentMotivation(null);
    setFormData({ nom_motivation: '' });
    setIsModalOpen(true);
  };
  ...
  const confirmDelete = async () => {
    try {
      await motivationService.delete(deleteConfirmation);
      toast.success('Motivation supprimée avec succès');
      loadMotivations();
    } catch (error) {
      toast.error(error.response?.data?.message || 'Une erreur est survenue');
    } finally {
    ...
    return (
      <div className="flex justify-center items-center h-full">
        <div className="relative">
          ...
          <HeartIcon className="w-8 h-8 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{ color: COLORS.primary }} />
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 min-h-screen" style={{ backgroundColor: COLORS.bg }}>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold" style={{ color: COLORS.text }}>Gestion des Motivations</h1>
            <p className="text-sm" style={{ color: COLORS.textLight }}>Définissez les motivations disponibles</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold" style={{ color: COLORS.text }}>Liste des Motivations</h2>
        </div>
        <button
          onClick={handleAdd}
          ...
          <table className="min-w-full">
            <thead style={{ backgroundColor: COLORS.accent }}>
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold" style={{ color: COLORS.text }}>Nom de la Motivation</th>
                <th className="px-6 py-4 text-right text-xs font-semibold" style={{ color: COLORS.text }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {motivations.map((motivation, index) => (
                <tr key={motivation.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors" style={{ borderColor: COLORS.border }}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: COLORS.statGreen }}>
                        <HeartIcon className="w-5 h-5" style={{ color: COLORS.statGreenText }} />
                      </div>
                      <span className="ml-3 text-sm font-medium" style={{ color: COLORS.text }}>{motivation.nom_motivation}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="relative">
                      <button
                        onClick={() => setOpenMenuId(openMenuId === motivation.id ? null : motivation.id)}
                      ...
                      {openMenuId === motivation.id && (
                        <div className="absolute right-0 mt-2 w-48 rounded-xl shadow-lg border z-20 overflow-hidden" style={{ backgroundColor: COLORS.card, borderColor: COLORS.border }}>
                          <button
                            onClick={() => { handleEdit(motivation); setOpenMenuId(null); }}
                          ...
                          <button
                            onClick={() => { handleDeleteClick(motivation.id); setOpenMenuId(null); }}
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full mx-auto my-8 overflow-hidden">
            <div className="px-6 py-5 border-b flex items-center justify-between" style={{ borderColor: COLORS.border, backgroundColor: COLORS.accent }}>
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-semibold" style={{ color: COLORS.text }}>{currentMotivation ? "Modifier la motivation" : "Ajouter une motivation"}</h3>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 max-h-[70vh] overflow-y-auto">
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-semibold mb-2" style={{ color: COLORS.text }}>Nom de la Motivation</label>
                  <input
                    type="text"
                    name="nom_motivation"
                    value={formData.nom_motivation}
              ...
              <div className="flex justify-end gap-3 mt-6 pt-6 border-t" style={{ borderColor: COLORS.border }}>
                ...
                <button type="submit" className="px-5 py-2.5 rounded-xl text-white font-semibold shadow-md hover:shadow-lg transition-all" style={{ background: COLORS.gradient }}>{currentMotivation ? "Modifier" : "Ajouter"}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 overflow-y-auto p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            <div className="p-6">
              ...
              <p className="text-center mb-6" style={{ color: COLORS.textLight }}>Êtes-vous sûr de vouloir supprimer cette motivation ?</p>
