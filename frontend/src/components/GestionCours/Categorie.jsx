...
import { EllipsisHorizontalIcon, PencilIcon, TrashIcon, TagIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const Categorie = () => {
  ...
  const handleEdit = (categorie) => {
    ...
    setFormData({
      ...
      id_type_cours: categorie.id_type_cours?.toString() || '',
  ...
  const handleAdd = () => {
    ...
    setFormData({ nom_categorie: '', id_type_cours: '', min_age: '', max_age: '' });
    setIsModalOpen(true);
  };
  ...
  const confirmDelete = async () => {
    try {
      await categorieService.delete(deleteConfirmation);
    ...
    return (
      <div className="flex justify-center items-center h-full">
        <div className="relative">
          ...
          <TagIcon className="w-8 h-8 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{ color: COLORS.primary }} />
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 min-h-screen" style={{ backgroundColor: COLORS.bg }}>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold" style={{ color: COLORS.text }}>Gestion des Catégories</h1>
            <p className="text-sm" style={{ color: COLORS.textLight }}>Définissez les catégories de cours</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold" style={{ color: COLORS.text }}>Liste des Catégories</h2>
        </div>
        <button
          onClick={handleAdd}
          ...
          <table className="min-w-full">
            <thead style={{ backgroundColor: COLORS.accent }}>
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold" style={{ color: COLORS.text }}>Nom de la catégorie</th>
                <th className="px-6 py-4 text-left text-xs font-semibold" style={{ color: COLORS.text }}>Type de cours</th>
                <th className="px-6 py-4 text-left text-xs font-semibold" style={{ color: COLORS.text }}>Âge Min</th>
                <th className="px-6 py-4 text-left text-xs font-semibold" style={{ color: COLORS.text }}>Âge Max</th>
                <th className="px-6 py-4 text-right text-xs font-semibold" style={{ color: COLORS.text }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {categories.map((categorie, index) => (
                <tr key={categorie.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors" style={{ borderColor: COLORS.border }}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: COLORS.statBlue }}>
                        <TagIcon className="w-5 h-5" style={{ color: COLORS.statBlueText }} />
                      </div>
                      <span className="ml-3 text-sm font-medium" style={{ color: COLORS.text }}>{categorie.nom_categorie}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm" style={{ color: COLORS.textLight }}>
                    {getTypeCoursName(categorie.id_type_cours)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="relative">
                      ...
                      {openMenuId === categorie.id && (
                        <div className="absolute right-0 mt-2 w-48 rounded-xl shadow-lg border z-20 overflow-hidden" style={{ backgroundColor: COLORS.card, borderColor: COLORS.border }}>
                          <button
                            onClick={() => { handleEdit(categorie); setOpenMenuId(null); }}
                          ...
                          <button
                            onClick={() => { handleDeleteClick(categorie.id); setOpenMenuId(null); }}
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full mx-auto my-8 overflow-hidden">
            <div className="px-6 py-5 border-b flex items-center justify-between" style={{ borderColor: COLORS.border, backgroundColor: COLORS.accent }}>
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-semibold" style={{ color: COLORS.text }}>{currentCategorie ? "Modifier la catégorie" : "Ajouter une catégorie"}</h3>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 max-h-[70vh] overflow-y-auto">
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-semibold mb-2" style={{ color: COLORS.text }}>Nom de la catégorie</label>
                  <input
                    type="text"
                    name="nom_categorie"
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: COLORS.text }}>Âge Min</label>
                    <input
                      type="number"
                      name="min_age"
                  ...
                  <div>
                    <label className="block text-sm font-semibold mb-2" style={{ color: COLORS.text }}>Âge Max</label>
                    <input
                      type="number"
                      name="max_age"
              ...
              <div className="flex justify-end gap-3 mt-6 pt-6 border-t" style={{ borderColor: COLORS.border }}>
                ...
                <button type="submit" className="px-5 py-2.5 rounded-xl text-white font-semibold shadow-md hover:shadow-lg transition-all" style={{ background: COLORS.gradient }}>{currentCategorie ? "Modifier" : "Ajouter"}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 overflow-y-auto p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            <div className="p-6">
              ...
              <p className="text-center mb-6" style={{ color: COLORS.textLight }}>Êtes-vous sûr de vouloir supprimer cette catégorie ?</p>
