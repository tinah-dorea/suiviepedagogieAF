...
const Niveau = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submitData = {
        nom_niveau: formData.nom_niveau.trim().toUpperCase(),
        sous_niveau: formData.sous_niveau ? parseInt(formData.sous_niveau, 10) : null
      };
  ...
  const handleAdd = () => {
    ...
    setFormData({ nom_niveau: '', sous_niveau: '' });
    setIsModalOpen(true);
  };
  ...
  const confirmDelete = async () => {
    try {
      await niveauService.delete(deleteConfirmation);
    ...
    <div className="p-6 min-h-screen" style={{ backgroundColor: COLORS.bg }}>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold" style={{ color: COLORS.text }}>Gestion des Niveaux</h1>
            <p className="text-sm" style={{ color: COLORS.textLight }}>Définissez les niveaux disponibles</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold" style={{ color: COLORS.text }}>Liste des Niveaux</h2>
        </div>
        <button
          onClick={handleAdd}
          ...
          <table className="min-w-full">
            <thead style={{ backgroundColor: COLORS.accent }}>
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold" style={{ color: COLORS.text }}>Code</th>
                <th className="px-6 py-4 text-left text-xs font-semibold" style={{ color: COLORS.text }}>Nom du Niveau</th>
                <th className="px-6 py-4 text-left text-xs font-semibold" style={{ color: COLORS.text }}>Sous-niveau</th>
                <th className="px-6 py-4 text-right text-xs font-semibold" style={{ color: COLORS.text }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {niveaux.map((niveau, index) => (
                <tr key={niveau.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors" style={{ borderColor: COLORS.border }}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: COLORS.statGreen }}>
                        <AcademicCapIcon className="w-5 h-5" style={{ color: COLORS.statGreenText }} />
                      </div>
                      <span className="ml-3 text-sm font-medium" style={{ color: COLORS.text }}>{niveau.nom_niveau}</span>
                    </div>
                  </td>
                    ...
                    <div className="relative">
                      ...
                      {openMenuId === niveau.id && (
                        <div className="absolute right-0 mt-2 w-48 rounded-xl shadow-lg border z-20 overflow-hidden" style={{ backgroundColor: COLORS.card, borderColor: COLORS.border }}>
                          <button
                            onClick={() => { handleEdit(niveau); setOpenMenuId(null); }}
                          ...
                          <button
                            onClick={() => { handleDeleteClick(niveau.id); setOpenMenuId(null); }}
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full mx-auto my-8 overflow-hidden">
            <div className="px-6 py-5 border-b flex items-center justify-between" style={{ borderColor: COLORS.border, backgroundColor: COLORS.accent }}>
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-semibold" style={{ color: COLORS.text }}>{currentNiveau ? "Modifier le niveau" : "Ajouter un niveau"}</h3>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 max-h-[70vh] overflow-y-auto">
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-semibold mb-2" style={{ color: COLORS.text }}>Nom du Niveau</label>
                  <input
                    type="text"
                    name="nom_niveau"
                    value={formData.nom_niveau}
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 rounded-xl border-2 focus:outline-none focus:ring-2 focus:ring-opacity-50 transition-all uppercase"
                    style={{ borderColor: COLORS.border, backgroundColor: COLORS.bg }}
                    required
                  />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-2" style={{ color: COLORS.text }}>Sous-niveau</label>
                  <input
                    type="number"
                    name="sous_niveau"
              ...
              <div className="flex justify-end gap-3 mt-6 pt-6 border-t" style={{ borderColor: COLORS.border }}>
                ...
                <button type="submit" className="px-5 py-2.5 rounded-xl text-white font-semibold shadow-md hover:shadow-lg transition-all" style={{ background: COLORS.gradient }}>{currentNiveau ? "Modifier" : "Ajouter"}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 overflow-y-auto p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
            <div className="p-6">
              ...
              <p className="text-center mb-6" style={{ color: COLORS.textLight }}>Êtes-vous sûr de vouloir supprimer ce niveau ?</p>
